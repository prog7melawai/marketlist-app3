  ...
  methods: {
    ...
    async submitMaster() {
      try {
        ...
        if(this.url === 'satuan') {
          this.body = {
            kode_satuan: this.master.kode,
            nama_satuan: this.master.name
          }
        } else {
          this.body = {
            kode_jenis: this.master.kode,
            nama_jenis: this.master.name
          }
        }

        unit.push(this.body)
        const { data } = await axios.post(`/master${this.url}`, unit, {
